...
namespace dotNetLab.Data.Orm
{
    public class OrmDBPlatform : IDisposable
    {
        ...
        /// 初始化连接远程的数据库
        /// </summary>
        void Init(Type type_DBEngine, String ip, int port, String DBName = "shikii", String usrName = "root", String pwd = "123", params Assembly[] EntitySourceAssemblies)
        {
            ThisDBManager = new DBManager();
        ...
        public Exception USave(EntityBase entity)
        {
            try
            {
                ...
                foreach (var item in pifs)
                {
                    ...
                    {
                        List<Attribute> lst_Attributes = item.GetCustomAttributes().ToList();
                        DBArrayAttribute arrMark = lst_Attributes.Find(x => x is DBArrayAttribute) as DBArrayAttribute;
                        if (arrMark != null)
                        {
                            String _value = AdonetContext.UniqueResult(String.Format("select {0} from {1} where {2}={3}", item.Name,
                                tableName, entity.PrimaryKeyPropertyName, entity.GetPrimaryKeyValue()));
                            string _secondValue = item.GetValue(entity).ToString();
                            item.SetValue(entity, String.Format("{0}\r\n{1}", _value, _secondValue));
                        }
            ...
            ) where T : EntityBase
        {
            ...
            sqlStringBuilder.Append(selectSQL);
            if (!fromSQL.ToLower().Contains(" from "))
...
//        void CheckTableColumnPosChanged(String TableName ,List<String> lstColNames, List<String> EntityColNames,
//            PropertyInfo[] pifs )
//        {

//            //可能列位置发生了变化 
//            if (lstColNames.Count == EntityColNames.Count
//                && EntityColNames.Except(lstColNames).Count() == 0)
//            {

//                for (int i = 0; i < lstColNames.Count; i++)
//                {
//                    if(!lstColNames[i].Equals(EntityColNames[i],StringComparison.OrdinalIgnoreCase))
//                    {
//                        int index = EntityColNames.IndexOf(lstColNames[i]);
//                        String extra = "";
//                        if(index == 0)
//                        {
//                            extra = " first ";
...
//                        else
//                        {
//                            extra = String.Format(" after {0} ", EntityColNames[index - 1]);
//                        }

//                        Type type = pifs[index].PropertyType;
//                        String  str = IdentifyEntityPropertyType(pifs[index], type);

//                        String attrStr = "";

//                        DBKeyAttribute attr = pifs[index].GetCustomAttribute<DBKeyAttribute>();
//                        if (attr != null)
//                            attrStr = attr.KeyDescription;
//                        if (pifs[index].Name.Equals("Id"))
//                        {
//                            attrStr = GetAutoIncrementMark();
//                            if (AdonetContext.GetType().Name.Equals("PostgreSQLEngine"))
//                                str = " serial ";
...
//                        if (AdonetContext is MySQLDBEngine)
//                        {
//                            AdonetContext.
//                                ExecuteNonQuery(
//                                String.Format("ALTER TABLE {0} MODIFY {1} {2} {3} ", TableName, lstColNames[i],str,extra));
//                        }
...
//            }
//        }
//        void CheckTableColumnNameChanged(String TableName, List<String> lstColNames, List<String> EntityColNames,
//           PropertyInfo[] pifs)
//        {
...
//                List<String> EntityColNames = new List<string>();
//                pifs.ToList().ForEach(x => EntityColNames.Add(x.Name));
//                if (recordEntityInfo)
//                {
//                    if(!TableManager.Keys.Contains(tableName))
//                    this.TableManager.Add(tableName, GetEntityInfo(pifs));
...
//                            CheckTableColumnPosChanged(tableName, lstColNames, EntityColNames,pifs   );
...
//                            //是否有增加属性名的情况
//                            if (pifs.Length > lstColNames.Count)
//                            {

//                                List<String> AddedColNameSet = EntityColNames.Except(lstColNames).ToList();

//                                for (int j = 0; j < AddedColNameSet.Count; j++)
//                                {
//                                    String item = AddedColNameSet[j];
//                                    String extra = "";
//                                    int nindex = EntityColNames.IndexOf(item);
//                                    if(nindex==0)
//                                    {
//                                          extra = " first ";
//                                    }

//                                    else if(nindex != EntityColNames.Count-1)
//                                    {
//                                       extra  = " after "+ EntityColNames[nindex - 1];
//                                    }

//                                    Type type = pifs[nindex].PropertyType;
//                                    str = IdentifyEntityPropertyType(pifs[nindex], type);

//                                    String attrStr = "";

//                                    DBKeyAttribute attr = pifs[nindex].GetCustomAttribute<DBKeyAttribute>();
//                                    if (attr != null)
//                                        attrStr = attr.KeyDescription;
//                                    if (pifs[nindex].Name.Equals("Id"))
//                                    {
//                                        attrStr = GetAutoIncrementMark();
//                                        if (AdonetContext.GetType().Name.Equals("PostgreSQLEngine"))
//                                            str = " serial ";
//                                    }
//                                    if (alterDB is MySQLDBEngine)
//                                        alterDB.AddColumn(tableName, pifs[nindex].Name, str + attrStr, extra);
//                                    else
//                                        alterDB.AddColumn(tableName, pifs[nindex].Name, str + attrStr);
//                                }
